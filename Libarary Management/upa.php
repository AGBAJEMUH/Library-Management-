...
do {
    if ( empty($name)  and  empty($email)  and   empty($number) ) {
        echo"
        <script>
        window.location.href = 'upa.php?id=$id';
    ...
    $sql= "UPDATE `volunteer` SET  `name` ='$name', `email` ='$email', `phone` ='$number' WHERE id = '$id'" ;
    $result = mysqli_query($conn, $sql);
...
<body>
    <div class="container my-5">
       ...
       <form method="post" action= "upa.php?id=<?php echo $id ?>">
        <input type="hidden" name="id" value="<?php echo $id; ?>">
         ...
         <div class="row mb-3">
            ...
            <div class="col-sm-3 d-grid">
                <a class="btn btn-outline-primary" role="button" href="dash.php?id=<?php echo $id; ?>">Cancel</a>
            </div>
         </div>
       </form>
</body>

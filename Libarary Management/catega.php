...
<body>
  ...
  <i class="fa-solid fa-bookmark fa-2x col-sm-1 "></i>
    ...
    <?php
    echo "<h1 class='col-sm-6 '>$name</h1>";
    echo "<h3 class='col-sm-6 text-secondary'>$catdes</h3>";
    ?>
    </div>
<form action="catega.php?id=<?php echo $id;?>&name=<?php echo $name;?>" method="post">
...
<?php
    ...
    while($row = mysqli_fetch_assoc($result)){
        ?>
        <div class= "btn col-sm-2 container my-5 offset-sm-1 btn-outline-dark" ><img src="stylesheet/images/icon1-removebg-preview.png" style="width: 100%;"alt=""><br><a class="btn" href= "booka.php?id=<?php echo $id; ?>&name=<?php echo $row['bname'];?>&category=<?php echo $name; ?>"><b style= "font-size:100%;color: black;"><?php echo $row['bname']?>(<?php echo $row['quantity'];?>)</b></a></div>
        <?php
    }
    ?>

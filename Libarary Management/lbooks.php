  ...
  <i class="fa-solid fa-bookmark fa-2x col-sm-1 "></i>
    ...
    <div class="collapse navbar-collapse" id="">
      <ul class="nav nav-underline offset-sm-5" >
        ...
        <li class="nav-link">
        <a class="nav-link active" aria-current="page" href= "booksa.php?id=<?php echo $id; ?>">ALL BOOKS</a>
        </li>
      </ul>
    </div>
  </div>
    ...
    <div class2="row mb-1 container my-5 offset-sm-6">
            ...
            <a class="col btn btn-outline-success btn-success my-1" href= "booksa.php?id=<?php echo $id; ?>"><b style= "font-size:75%; color: black;">ALL BOOKS</b></a>
    </div>
    </div>-->
    <br><br>
    <div class="container my-5">
        ...
        <br>
        <h3 class='col-sm-6'>These are the list of books that are available at the moment <br>Feel free to Borrow books or read books online</h3>

    </div>
<form action="lbooks.php?id=<?php echo $id;?>" method="post">
...
if(isset($_POST['ok'])){
    $search = $_POST['search'];
    $sql = "select * from book where bname like '%$search%' and quantity > 0";
...
<?php
    ...
    $errorMessag = "Book not Available in This Library or is out of stock";
}
}
?>
</div>
<?php
    ...
    <?php
    $sql = "select * from book where bname!='null' and quantity > 0";
    $result = mysqli_query($conn, $sql);
    while($row = mysqli_fetch_assoc($result)){
        ?>
